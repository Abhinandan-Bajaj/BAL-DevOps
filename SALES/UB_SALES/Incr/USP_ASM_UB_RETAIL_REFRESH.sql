...
ALTER PROC [dbo].[USP_ASM_UB_RETAIL_REFRESH] AS
BEGIN
--***************************START************************************************
/*******************************************HISTORY**************************************************/
/*--------------------------------------------------------------------------------------------------*/
...
/*	2025-04-02 	|	Lachmanna		        | First mode Source and subsource  Changes 		*/
...
WHERE
--CAST(RH.DOCDATE AS DATE) BETWEEN '2021-01-01' AND '2022-12-31' AND 
RH.DOCTYPE IN(141,1000079,1000317,1012739,441) 
AND CAST(RH.IMPORTEDDATE AS DATE)>= CAST((SELECT MAX(IMPORTEDDATE) FROM ASM_UB_RETAIL_DIM) AS DATE)
...
AND CAST(RH.IMPORTEDDATE AS DATE)>= CAST((SELECT MAX(IMPORTEDDATE) FROM ASM_UB_RETAIL_STG) AS DATE) -- tO FIX INCR ISSUE
GROUP BY
   CM.CODE,
   IM.CODE+IV.CODE,
...
and 
CAST(HEADER.IMPORTEDDATE AS DATE)>= CAST((SELECT MAX(IMPORTEDDATE) FROM ASM_UB_ADDONS_FACT) AS DATE)
--CAST(HEADER.DocDate AS DATE)>='2020-01-01'
