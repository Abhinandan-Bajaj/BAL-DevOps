...
2025-09-16 	|	Richa Mishra 	| Handled logic of repeted jobs for deleted JC    */
...
DECLARE @SPID INT = @@SPID,
        @sp_name VARCHAR(128) = '[USP_ASM_MC_SERVICE_SERVICE_INCLOAD]';
...
DECLARE @StartDate_utc DATETIME = GETDATE(),
            ...
            @Duration_sec bigint,
			@Duration varchar(15),
			 @table_name VARCHAR(128) = 'ASM_MC_SERVICE_STG',
            @SourceCount BIGINT,  
            @TargetCount BIGINT, 			
            @Status VARCHAR(10),
...
--CAST(SH.DOCDATE AS DATE) BETWEEN '2022-04-01' AND DATEADD(DAY, -1, GETDATE())
...
and t1.FK_Itemid not in (select CODE from DM_CodeInclusionExclusion_Master where TypeFlag='Itemid_001' and IncORExc='Exclude')  --- Excluding Part codes for US-7307
...
/*  ---commenting out as no need to see multiple Repeat type as not showing multiple repeted cases
;WITH multi_type_docs AS (
    SELECT t1.*,
           COUNT( t1.[type]) OVER (PARTITION BY t1.FK_DOCID) AS type_count,
           ROW_NUMBER() OVER (PARTITION BY t1.FK_DOCID ORDER BY repeated_from_docdate DESC) AS rn
    FROM #repeatcases t1
    WHERE cnt > 0
),
dedup AS (
    SELECT 
        ...
        [type],
        ROW_NUMBER() OVER (
            PARTITION BY FK_DOCID 
            ORDER BY repeated_from_docdate DESC, repeated_from_docname DESC
        ) AS pick_one
    FROM multi_type_docs
    WHERE type_count > 1
)
SELECT *
...
AND B.AMC_DATE IS NULL
 
PRINT 'CWI AMC Date updated' +       CONVERT(VARCHAR, DATEADD(MINUTE, 330, GETDATE()))
 
--To update the historical one month AMC date against chassis
;WITH DELTA AS (
SELECT TYPE,FK_IBID,MAX(DOCDATE) AS DOCDATE FROM ASM_MC_CWI_STG
WHERE TYPE = 'AMC'
AND DOCDATE BETWEEN DATEADD(MONTH, -1, CAST(GETDATE() AS date)) AND CAST(GETDATE() AS date)
GROUP BY TYPE,FK_IBID
),
AMC_DATE AS(
SELECT DISTINCT
    F.FK_IBID,
...
WHERE F.FK_CONTRACTTYPEID = 40
and A.Type = 'AMC'
AND F.BILLEDDATETIME BETWEEN DATEADD(MONTH, -1, CAST(GETDATE() AS date)) AND CAST(GETDATE() AS date))
SELECT * INTO #AMC_TBL_DELTA
FROM AMC_DATE
UPDATE B
SET B.Amc_date=A.AMC_DATE
FROM ASM_MC_SERVICE_STG B
INNER JOIN #AMC_TBL_DELTA A
ON B.FK_IBID=A.FK_IBID
AND B.BILLEDDATETIME = A.BILLEDDATETIME
AND B.FK_DOCID = A.FK_DOCID
AND B.FK_CONTRACTTYPEID = 40
...
drop table #AMC_TBL
drop table #AMC_TBL_DELTA
print('Temporary tables dropped');
PRINT 'Script Execution completed' +       CONVERT(VARCHAR, DATEADD(MINUTE, 330, GETDATE()))

PRINT 'Audit Target count exc Started ' +       CONVERT(VARCHAR, DATEADD(MINUTE, 330, GETDATE()));

SELECT @TargetCount =  COUNT(1) FROM ASM_MC_SERVICE_FACT where IMPORTEDDATE > @MAXDATESTG
