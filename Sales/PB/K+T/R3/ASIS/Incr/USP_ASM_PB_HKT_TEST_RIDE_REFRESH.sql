...
GO
Create PROC [dbo].[USP_ASM_PB_HKT_TEST_RIDE_REFRESH] AS
BEGIN
...
SELECT
 ...
 ----------------------------------------------------------------------------------------------------------------------------------------------
    ...
    AND CAST( LSQ_TESTRIDE.CREATEDON AS DATE)  >=cast ((SELECT MAX(IMPORTEDDATE) FROM ASM_PB_HK_TEST_RIDE_FACT where FK_TYPE_ID='10008') as date)
  ) TMP
	GROUP BY 
...
WHERE LSQ_PACTEXTBASE.ActivityEvent=12002 
	--and LSQ_PEXTBASE.mx_BU_sub_type='TRM'
    AND LSQ_PEXTBASE.mx_Dealer_Assignment_Date  IS NOT NULL
    AND CAST( FIRST_FOLLOWUP.FirstfollowupDate AS DATE)   >=cast ((SELECT MAX(IMPORTEDDATE) FROM ASM_PB_HK_FOLLOWUP_FACT) as date)
  ) TMP
	GROUP BY 
DEALERCODE, SKU,FK_DEALERCODE, FK_SKU,  FK_TYPE_ID,DATE,ENQUIRYLINEID,
 FK_ENQUIRYDOCID,COMPANYTYPE,BRANCHID,LASTUPDATEDDATETIME,IMPORTEDDATE,CDMS_BATCHNO ,TARGETQUANTITY,PERIODNAME
...
SELECT
 ...
 ----------------------------------------------------------------------------------------------------------------------------------------------
	...
	--and LSQ_PEXTBASE.mx_BU_sub_type='TRM'
    AND LSQ_PEXTBASE.mx_Dealer_Assignment_Date  IS NOT NULL
    AND CAST( LSQ_PEXTBASE.mx_Enquiry_update_date_and_time AS DATE)   >=cast ((SELECT ISNULL(MAX(IMPORTEDDATE), '2023-04-01') FROM ASM_PB_HK_TEST_RIDE_FACT
	where FK_TYPE_ID='10011') as date)
  ) TMP
	GROUP BY 
DEALERCODE, SKU,FK_DEALERCODE, FK_SKU,  FK_TYPE_ID,DATE,ENQUIRYLINEID,
 FK_ENQUIRYDOCID,COMPANYTYPE,BRANCHID,LASTUPDATEDDATETIME,IMPORTEDDATE,CDMS_BATCHNO ,TARGETQUANTITY,PERIODNAME
...
EXEC USP_ASM_PB_T_TEST_RIDE_REFRESH
