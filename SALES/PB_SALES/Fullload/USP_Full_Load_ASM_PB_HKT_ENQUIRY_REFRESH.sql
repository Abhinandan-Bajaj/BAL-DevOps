...
/*  2025-10-28 	|	Lachmanna		        | Newly Added CRE Followup  and   Sales Person fron user      */
/*--------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------*/
/*******************************************HISTORY**************************************************/
--1. Enquiry Dim & Fact:
...
and Try_Cast(CreatedON as DATE)>= ( select  ISNULL(DATEADD(DAY, -2, MAX(EnquiryDate)), '2021-01-01')  from ASM_PB_HK_ENQUIRY_DIM_LSQ_STG)
...
SELECT
  ...
  LSQ_PBASE.ProspectID as FK_ENQUIRYDOCID, 
   ...
   FROM
    ...
    WHERE LSQ_PACTEXTBASE.ActivityEvent=12002 and LSQ_PBASE.mx_BU='PB'
    And LSQ_PEXTBASE.mx_Dealer_Assignment_Date is not null
	AND CAST(DATEADD(mi,30,(DATEADD(hh,5,LSQ_PBASE.Modifiedon))) AS DATE) BETWEEN '2022-04-01' AND '2025-06-08'
 -- AND DATEADD(mi,30,(DATEADD(hh,5,LSQ_PBASE.Modifiedon))) >=@ASMStglsq_IMPORTEDDATE
  ) TMP
	GROUP BY 
DEALERCODE, SKU,FK_DEALERCODE, FK_SKU,  FK_TYPE_ID,DATE,ENQUIRYLINEID,
