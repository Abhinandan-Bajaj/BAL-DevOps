...
CREATE PROC [dbo].[USP_ASM_PB_SERVICE_RETAIL_FULLLOAD] AS
/*******************************************HISTORY**************************************************/
/*--------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------*/
...
/*******************************************HISTORY**************************************************/
BEGIN
TRUNCATE TABLE [dbo].[ASM_PB_SERVICE_RETAIL_STG]
TRUNCATE TABLE [dbo].[ASM_PB_RETAIL_FLAGS]
TRUNCATE TABLE [dbo].[ASM_PB_RETAIL_FLAGS_UNION]
TRUNCATE TABLE [dbo].[ASM_PB_FREE_SERVICE]
...
WHERE CAST(RH.DOCDATE AS DATE)>='2012-04-01'
...
Insert into ASM_PB_SERVICE_RETAIL_STG
	( Docdate, 
	FK_ibid,
	FK_Branchid,
	FK_Modelid,
	...
	select 
	 TRY_CAST(RetailDate AS DATE),
	TRY_CAST(ib.ibid AS DECIMAL(10, 0)),
	TRY_CAST(bm.branchid AS DECIMAL(10, 0)),
	sh.fk_modelid,
	oc.dealercode_new,
	'Command Area' AS SALESCHANNEL,
	'NR' AS Return_Flag,
	102 as Service_Retail_TypeIdentifier,
	GETDATE() AS Refresh_Date
	,DATEADD(DAY, 45, TRY_CAST(RetailDate AS DATE) ) AS [1stPS_EXPIRY1_Date_BB]--BB
 ,DATEADD(DAY, 11, TRY_CAST(RetailDate AS DATE)) AS [2ndPS_EXPIRY1_Date_BB]
 ,DATEADD(DAY, 60, TRY_CAST(RetailDate AS DATE)) AS [1stPS_EXPIRY2_Date_BB]
 ,DATEADD(DAY, 14, TRY_CAST(RetailDate AS DATE)) AS [2ndPS_EXPIRY2_Date_BB]
 ,CAST(NULL AS DATE) AS FPS_SERVICE_DATE_BB
 ,CAST(NULL AS DATE) AS SPS_SERVICE_DATE_BB

	FROM ASM_PB_OLD_CHASSIS oc
left JOIN ( SELECT *
    FROM ( SELECT *,
           ROW_NUMBER() OVER (PARTITION BY name ORDER BY importeddate) AS rn
    FROM INSTALL_BASE_MASTER ibb
	 WHERE ibb.name IN (SELECT chassisno FROM ASM_PB_OLD_CHASSIS) 
) ib_sub  where   ib_sub.rn = 1
)ib  ON oc.chassisno = ib.name
left join  BRANCH_MASTER bm on oc.branchcode=bm.code
left join 
	 ( SELECT *
    FROM ( SELECT *,
           ROW_NUMBER() OVER (PARTITION BY fk_IBID ORDER BY importeddate) AS rn
    FROM ASM_PB_SERVICE_STG SH
) SH_sub  where   SH_sub.rn = 1
)sh  on ib.ibid=sh.fk_ibid
