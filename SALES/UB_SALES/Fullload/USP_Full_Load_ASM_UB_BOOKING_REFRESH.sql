...
ALTER  PROC [dbo].[USP_Full_Load_ASM_UB_BOOKING_REFRESH] AS
BEGIN
--***************************START************************************************
/*******************************************HISTORY**************************************************/
/*--------------------------------------------------------------------------------------------------*/
...
/*	2025-03-125 	|	Lachmanna		| First Mode Source and sub source  Addition	*/
/*--------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------*/
/*******************************************HISTORY**************************************************/
--***************************START*****************************
--1. Booking Dim:
Truncate table ASM_UB_BOOKING_DIM
Truncate table ASM_UB_BOOKING_FACT
...
WHERE 
CAST(BOOKING_HEADER.DOCDATE AS DATE) BETWEEN '2021-04-01' AND Cast(Getdate()-1 as date) AND
--BOOKING_HEADER.STATUS = 'Open' AND 
BOOKING_HEADER.DOCTYPE=1012737 --AND BOOKING_HEADER.IMPORTEDDATE>(SELECT MAX(IMPORTEDDATE) FROM ASM_UB_BOOKING_DIM)
...
WHERE 
CAST(BH.DOCDATE AS DATE) BETWEEN '2021-04-01' AND Cast(Getdate()-1 as date)  AND 
--BH.STATUS = 'Open' AND 
BH.DOCTYPE = 1012737 --AND BH.IMPORTEDDATE>(SELECT MAX(IMPORTEDDATE) FROM ASM_UB_BOOKING_FACT)
...
update B set B.FK_DEALERCODE=C.PK_DEALERCODE from ASM_UB_BOOKING_FACT B INNER JOIN ASM_UB_DEALER_MASTER_DIM C on (B.DEALERCODE=C.DEALERCODE);


--******************************************************************************************************************

-----------------Update LEADTYPE and SALESPERSON------------
