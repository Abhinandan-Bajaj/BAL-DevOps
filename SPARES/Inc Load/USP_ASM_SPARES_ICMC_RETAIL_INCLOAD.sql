...
CREATE PROC [dbo].[USP_ASM_SPARES_ICMC_RETAIL_INCLOAD]
AS
BEGIN
/*******************************************INC LOAD**************************************************/
/*--------------------------------------------------------------------------------------------------*/
...
/*  2025-04-09  |       Aswani    |        Included the logic to update the GOVT_ID                 */
...
DECLARE @MAXDATESTG DATETIME2(7)= (SELECT Dateadd(Day,-1,MAX(docdate_sales)) FROM ASM_ICMC_SPARES_RETSALES_STG);
...
AND RSALES.docdate_sales  >= @MAXDATESTG
) A
)
...
DECLARE @MAXDATESTG1 DATETIME2(7)= (SELECT Dateadd(Day,-1,MAX(docdate_sales)) FROM ASM_ICMC_SPARES_RETSALES_STG);
...
AND RADD.RETAILER_DATE  >= @MAXDATESTG1
)
...
-------------------------------Checking for Null GOVT_ID, Update if it is not null---------------------------

SELECT A.SOLD_TO_DEALER_OR_DISTRIBUTOR, A.RETAILER_CODE, B.GOVT_ID
INTO #MissedGovtID
FROM 
(
SELECT DISTINCT SOLD_TO_DEALER_OR_DISTRIBUTOR, RETAILER_CODE FROM ASM_ICMC_SPARES_RETAIL_SALES_FACT 
WHERE  CONCAT_WITH_GOVT_ID IS NULL
)A
JOIN 
(
SELECT CODE, GOVT_ID
...
ON A.RETAILER_CODE = B.CODE

UPDATE A 
SET A.CONCAT_WITH_GOVT_ID = B.GOVT_ID
FROM ASM_ICMC_SPARES_RETAIL_SALES_FACT A 
INNER JOIN #MissedGovtID B ON A.SOLD_TO_DEALER_OR_DISTRIBUTOR = B.SOLD_TO_DEALER_OR_DISTRIBUTOR AND A.RETAILER_CODE = B.RETAILER_CODE
WHERE CONCAT_WITH_GOVT_ID IS NULL

----------------------------------------------------------------------------------------------

DROP TABLE #MissedGovtID

PRINT('Retail sales data inc load is loaded for Retailer screen successfully')
