...
GO
ALTER PROC [dbo].[USP_Full_Load_ASM_PB_HK_STOCK_REFRESH] AS
BEGIN
--*********************START************************
--Stock Fact:
Truncate table ASM_PB_HK_STOCK_FACT
...
WHERE 
  ...
  --Cast(VS.EXECUTIONDATE as date)>(SELECT MAX(DATE) from ASM_PB_STOCK_FACT)
GROUP BY
  VS.COMPANYCODE,
...
FROM
   VEHICLE_STOCK_DATA VS INNER JOIN COMPANY_MASTER CM ON (VS.COMPANYCODE=CM.CODE AND (CM.COMPANYTYPE = 2 AND CM.COMPANYSUBTYPE is null))
WHERE 
    CAST(VS.EXECUTIONDATE AS DATE) BETWEEN '2020-04-01' AND  '2025-06-08'
  --Cast(VS.EXECUTIONDATE as date)>(SELECT MAX(DATE) from ASM_PB_STOCK_FACT)
GROUP BY
  VS.COMPANYCODE,
  VS.MODELCODE+LEFT(RIGHT(VS.COLOR,3),2),
