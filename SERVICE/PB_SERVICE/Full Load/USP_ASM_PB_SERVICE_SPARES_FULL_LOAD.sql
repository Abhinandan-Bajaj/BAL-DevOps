...
GO
ALTER PROC [dbo].[USP_ASM_PB_SERVICE_SPARES_FULL_LOAD]  AS 
/*******************************************HISTORY**************************************************/
/*--------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------*/
...
/*       08-05-25 | RICHA | Commenting out Stock data for optimization 	*/
...
TRUNCATE TABLE [dbo].[ASM_PB_SPARES_STOCK_STG];
...
FROM (
    SELECT DISTINCT 
        ...
        LAG(sd.netstock) OVER (
            PARTITION BY sd.companycode, sd.partcode 
            ORDER BY sd.todate
        ) AS previous_netstock,
		sd.importeddate
    FROM SPARE_STOCK_DATA_NEW sd 
    JOIN COMPANY_MASTER cm 
        ON sd.companycode = cm.code 
