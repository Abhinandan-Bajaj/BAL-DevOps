...
ALTER PROC [dbo].[USP_ASM_CV_SERVICE_SERVICE_INCLOAD] AS 
BEGIN
/*******************************************HISTORY**************************************************/
/*--------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------*/
...
/*	2024-08-11 	|	Dewang Makani	    | P3 EOH screen logic addition */
/*--------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------*/
/*******************************************HISTORY**************************************************/

DECLARE @MAXDATESTG DATETIME2(7)= (SELECT MAX(IMPORTEDDATE) FROM ASM_CV_SERVICE_STG)
...
WHERE CAST(SH.DOCDATE AS DATE) >= CAST(DATEADD(D,-90,GETDATE()) AS DATE) AND SH.HEADERID NOT IN (SELECT DISTINCT FK_DOCID FROM #JC_HEADER)
AND SH.IBID IS NOT NULL
AND SH.CANCELLATIONDATE IS NULL
--AND SL.ISCLOSED<>0
...
Delete from ASM_CV_SERVICE_STG Where DOCDATE>Cast(DATEADD(mi,30,(DATEADD(hh,5,getdate()))) - 1 as date);
---------------------------------------DEDUP
...
print('Updating Cancellation date in ASM_CV_SERVICE_STG table')
...
print('Cancellation date updated in ASM_CV_SERVICE_STG table');;
