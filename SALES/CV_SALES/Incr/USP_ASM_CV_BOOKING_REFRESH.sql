...
ALTER PROC [dbo].[USP_ASM_CV_BOOKING_REFRESH] AS
BEGIN
...
--CAST(BOOKING_HEADER.DOCDATE AS DATE)>='2020-04-01' and
--(Qty = QtyPending AND QtyCancelled = 0) AND 
BOOKING_HEADER.DOCTYPE=1000386 AND
--BOOKING_HEADER.IMPORTEDDATE>(SELECT MAX(IMPORTEDDATE) FROM ASM_CV_BOOKING_DIM) 
CAST(BOOKING_HEADER.IMPORTEDDATE AS DATE) >= CAST((SELECT MAX(IMPORTEDDATE) FROM ASM_CV_BOOKING_DIM) AS DATE)  --UPDATED ON 12/08
 

--
Delete from ASM_CV_BOOKING_DIM Where Cast(BOOKINGDATE as Date)>Cast(DATEADD(mi,30,(DATEADD(hh,5,getdate())))-1 as date) -- --UTC_TO_IST_Changes_needed?
...
LEFT JOIN LSQ_ProspectActivity_ExtensionBase PAE ON (Cast(PAE.RelatedProspectId+','+PAE.ProspectActivityExtensionId as varchar(8000))=BHE.LMSBOOKINGID) and PAE.ActivityEvent=12003 --Added as part of LeadTypeChange
---and PAE.mx_Custom_48 IS NULL -- Removing for Bug fix
...
--CAST(BH.DOCDATE AS DATE)>='2020-04-01' AND
--(BL.Qty = BL.QtyPending AND BL.QtyCancelled = 0) AND 
BH.DOCTYPE=1000386  AND --AND --Applied doctype filter after discussion had with PawanB on 05.01.2023
--BH.IMPORTEDDATE>(SELECT MAX(IMPORTEDDATE) FROM ASM_CV_BOOKING_FACT) 
CAST(BH.IMPORTEDDATE AS DATE) >= CAST((SELECT MAX(IMPORTEDDATE) FROM ASM_CV_BOOKING_FACT) AS DATE) --UPDATED ON 12/08
GROUP BY
CM.CODE,
IM.CODE+IV.CODE,
...
DELETE FROM ASM_CV_BOOKING_FACT WHERE DATE>Cast(DATEADD(mi,30,(DATEADD(hh,5,getdate())))-1 as date)-- --UTC_TO_IST_Changes_needed?
