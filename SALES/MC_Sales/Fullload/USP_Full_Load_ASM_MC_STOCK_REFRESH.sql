...
ALTER PROC [dbo].[USP_Full_Load_ASM_MC_STOCK_REFRESH] AS
BEGIN


TRUNCATE TABLE ASM_MC_STOCK_FACT
--Stock Fact:
...
--INTO ASM_MC_STOCK_FACT
FROM
   VEHICLE_STOCK_DATA VS INNER JOIN COMPANY_MASTER CM ON (VS.COMPANYCODE=CM.CODE AND (CM.COMPANYTYPE=1))
    where CAST(VS.EXECUTIONDATE AS DATE) BETWEEN '2020-04-01' AND Cast(Getdate()-1 as date) 
GROUP BY
  VS.COMPANYCODE,
...
--update B set B.FK_SKU=C.PK_SKU from ASM_MC_STOCK_FACT B INNER JOIN ASM_MC_PRODUCT_SKU_DIM C on (B.SKU=C.SKU_CODE) -- (116763 records affected)
--update B set B.FK_MODEL=C.PK_Model_Code from ASM_MC_STOCK_FACT B INNER JOIN ASM_MC_PRODUCT_DIM C on (B.ModelCode=C.MODELCODE) -- (117266 records affected)
--update B set B.FK_DEALERCODE=C.PK_DEALERCODE from [dbo].ASM_MC_STOCK_FACT B INNER JOIN ASM_MC_DEALER_MASTER_DIM C on (B.DEALERCODE=C.DEALERCODE) -- (64571 records affected)
