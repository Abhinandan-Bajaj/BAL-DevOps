/****** Object:  StoredProcedure [dbo].[USP_Full_Load_ASM_PB_BOOKING_REFRESH]    Script Date: 6/13/2025 4:46:29 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
Alter PROC [dbo].[USP_Full_Load_ASM_PB_HKT_BOOKING_REFRESH] AS
BEGIN
/*******************************************HISTORY**************************************************/
/*--------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------*/
...
/*	2024-12-10 	|	Nikita L		| LSQ-MX_First_Source Addition                    			*/
/*	2025-03-19 	|	Lachmanna		| First Mode Source and sub source  Addition	*/
/*--------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------*/
/*******************************************HISTORY**************************************************/
--***************************START*****************************
Truncate table ASM_PB_HK_BOOKING_DIM
Truncate table ASM_PB_HK_BOOKING_FACT

--1. Booking Dim:
--Select count(*) from ASM_PB_BOOKING_DIM
--Truncate table ASM_PB_BOOKING_DIM
...
--AND BOOKING_HEADER.IMPORTEDDATE>(SELECT MAX(IMPORTEDDATE) FROM ASM_PB_BOOKING_DIM)
...
WHERE 
 CAST(BOOKING_HEADER.DOCDATE AS DATE) BETWEEN '2020-04-01' AND '2025-06-08' AND 
--BOOKING_HEADER.STATUS = 'Open' AND 
BOOKING_HEADER.DOCTYPE=1000050
--AND BOOKING_HEADER.IMPORTEDDATE>(SELECT MAX(IMPORTEDDATE) FROM ASM_PB_BOOKING_DIM)
...
--*************************************************************************
--2. Booking Fact:
--Select count(*) from ASM_PB_BOOKING_FACT
--Truncate table ASM_PB_BOOKING_FACT
...
LEFT JOIN LSQ_Prospect_Extension2Base PE2 ON (PE2.ProspectID=LSQ_PBASE.ProspectID)
WHERE 
  CAST(BH.DOCDATE AS DATE) BETWEEN '2020-04-01' AND   '2025-06-08' AND 
   BH.DOCTYPE=1000050
   --and   CM.CODE='0000013671'
GROUP BY
CM.CODE,
...
EXEC [USP_Full_Load_ASM_PB_T_BOOKING_REFRESH];
