...
set @ASMDimcdms_IMPORTEDDATE = CAST((SELECT MAX(IMPORTEDDATE) FROM ASM_PB_ENQUIRY_DIM_CDMS_STG)AS DATE);

declare @ASMDimlsq_IMPORTEDDATE date;
set @ASMDimlsq_IMPORTEDDATE = CAST((SELECT MAX(IMPORTEDDATE) FROM ASM_PB_ENQUIRY_DIM_LSQ_STG)AS DATE);

declare @ASMStgcdms_IMPORTEDDATE date;
set @ASMStgcdms_IMPORTEDDATE =CAST((SELECT MAX(IMPORTEDDATE) FROM ASM_PB_ENQUIRY_STG) AS DATE);


declare @ASMStglsq_IMPORTEDDATE date;
set @ASMStglsq_IMPORTEDDATE = CAST((SELECT MAX(IMPORTEDDATE) FROM ASM_PB_ENQUIRY_FACT_LSQ_STG)AS DATE);
...
and Try_Cast(CreatedON as DATE)>= ( select  ISNULL(DATEADD(DAY, -2, MAX(EnquiryDate)), '2023-01-01')  from ASM_PB_T_ENQUIRY_DIM)
  ...
  CASE WHEN UPPER(LSQ_PBASE.ProspectStage) IN ('BOOKED','ALLOCATED') THEN 'Booked'
    ...
    LATEST_FOLLOWUP.LatestIsCustomerContacted,
			...
			CASE 
				...
				WHEN CAST(DATEDIFF(MINUTE, COALESCE(LSQ_UTBASE.DueDate,LSQ_PEXTBASE.mx_Dealer_Assignment_Date ), FIRST_FOLLOWUP.FirstFollowupDate) AS INT) BETWEEN 181 AND 1440 THEN '3-24 Hrs'
