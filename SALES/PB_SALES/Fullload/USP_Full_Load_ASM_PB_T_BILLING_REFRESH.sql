/****** Object:  StoredProcedure [dbo].[USP_Full_Load_ASM_PB_T_BILLING_REFRESH]    Script Date: 5/23/2025 11:17:01 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
alter PROC [dbo].[USP_Full_Load_ASM_PB_T_BILLING_REFRESH] AS
BEGIN
--***************************START************************************************
/*******************************************HISTORY**************************************************/
/*--------------------------------------------------------------------------------------------------*/
...
TRUNCATE TABLE ASM_PB_T_BILLING_STG
...
COMPANY_MASTER.COMPANYTYPE = 2)-- and COMPANY_MASTER.companysubtype='PB_T')
--INNER JOIN SAP_ZSD_DEALER_REPOS REPOS ON REPOS.KUNNR = '0000'+SAP_BILLING.DEALERCODE AND REPOS.KATR6='TRM'
--testing<
-- Inline to ASM_PB_T_PRODUCT_DIM using derived MODELCODE
INNER JOIN (
    SELECT *, 
...
WHERE 
SAP_BILLING.DOCDATE BETWEEN '2025-06-09' AND Cast(Getdate()-1 as date)
...
NULL AS SALESPERSON,
BM.PK_BRANCHID as BRANCHCODE
--INTO ASM_TRIUMPH_BILLING_STG
FROM
   SAP_BILLING 
--INNER JOIN COMPANY_MASTER ON ('0000'+SAP_BILLING.DEALERCODE=COMPANY_MASTER.CODE AND --Comented as per req. of Nikita and approval from Adarsh and Kaushik 
--COMPANY_MASTER.COMPANYTYPE = 2 and COMPANY_MASTER.companysubtype='Triumph')
INNER JOIN SAP_ZSD_DEALER_REPOS REPOS ON REPOS.KUNNR = '0000'+SAP_BILLING.DEALERCODE AND REPOS.KATR6='TRM'
Left  JOIN sap_LIKP ON SAP_BILLING.DELIVERY=sap_LIKP.VBELN
Left JOIN ASM_TRIUMPH_BRANCH_MASTER_DIM BM on  sap_LIKP.KUNNR=BM.BRANCH_CODE
WHERE 
SAP_BILLING.DOCDATE BETWEEN '2022-04-01' AND '2025-06-08'
 --SAP_BILLING.DOCDATE> (SELECT MAX(DATE) from ASM_TRIUMPH_BILLING_STG)
AND [Distr.Chnl] IN ('55') AND DIVISION='B2' AND SAP_BILLING.CANCELLED<>'X'  -- (Change Division,'B2' is for MC)
GROUP BY
    '0000'+SAP_BILLING.DEALERCODE,
    SAP_BILLING.MATERIAL,
...
NULL AS TEHSILID,
NULL AS SALESPERSON
--INTO ASM_TRIUMPH_BILLING_PLAN_STG
FROM 
SAP_ZMPSS_ORDERS ZO
--testing<
...
NULL AS TEHSILID,
NULL AS SALESPERSON
--INTO ASM_TRIUMPH_PENDING_ORDER_STG
FROM 
