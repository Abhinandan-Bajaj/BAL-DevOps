...
ALTER PROC [dbo].[USP_Full_Load_ASM_CV_RETAIL_REFRESH] AS
BEGIN
...
/*  26/03/2025 |  Ashwini Ahire       |   first_source_leadtype, source , subsource against Retail */
...
DELETE FROM ASM_CV_RETAIL_DIM WHERE RETAILDATE>CAST(GETDATE()-1 AS DATE);
...
DELETE FROM ASM_CV_RETAIL_PREV_OWNVEH WHERE CAST(DOCDATE AS DATE)>CAST(GETDATE()-1 AS DATE)
...
DELETE FROM ASM_CV_RETAIL_EXCHANGE_OEM WHERE CAST(DOCDATE AS DATE)>CAST(GETDATE()-1 AS DATE)
...
GROUP BY
  ...
  DELETE FROM ASM_CV_RETAIL_STG WHERE DATE>CAST(GETDATE()-1 AS DATE)
...
/*
INSERT INTO ASM_CV_RETAIL_FACT
SELECT *,Null As Pincode,Null As Area, Null As First_Source_Lead_Type,Null As First_Mode_Source,Null As First_Mode_SubSource,Null As Salesperson_id, Null As Designation, null as Recurrence_id  FROM ASM_CV_RETAIL_STG
UNION
SELECT *,Null As Pincode,Null As Area, Null As First_Source_Lead_Type,Null As First_Mode_Source,Null As First_Mode_SubSource FROM ASM_CV_RETAIL_TARGET -- (555490 ROWS AFFECTED) */
