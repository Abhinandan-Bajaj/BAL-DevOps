...
ALTER PROC [dbo].[USP_ASM_MC_BOOKING_REFRESH] AS
BEGIN
/*******************************************HISTORY**************************************************/
/*--------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------*/
...
WHERE 
CAST(BH.IMPORTEDDATE AS DATE) >= CAST((SELECT MAX(IMPORTEDDATE) FROM ASM_MC_BOOKING_DIM)AS DATE)  
AND BH.DOCTYPE=135) A
...
First_Source_Lead_Type, -- For this field post update in Digital Leads SP
First_Mode_Source,
...
WHERE 
CAST(BH.IMPORTEDDATE AS DATE) >= CAST((SELECT MAX(IMPORTEDDATE) FROM ASM_MC_BOOKING_FACT) AS DATE) AND  
BH.DOCTYPE=135 
