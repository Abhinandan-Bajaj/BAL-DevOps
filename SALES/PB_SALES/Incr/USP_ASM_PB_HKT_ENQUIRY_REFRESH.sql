...
/*  2025-10-29 	|	Lachmanna		        | Added CRE Followup , sales person and   Followup Bucket Bug      */
/*--------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------*/
/*******************************************HISTORY**************************************************/
--1. Enquiry Dim & Fact:
...
----------------------------------------------------------------
    -- Audit Segment 1: ASM_PB_HK_ENQUIRY_DIM_LSQ_STG
----------------------------------------------------------------  
...
SELECT ProspectId
FROM LSQ_Prospect_Base WHERE DATEADD(MINUTE, 30, DATEADD(HOUR, 5, ModifiedOn)) >= @ASMDimlsq_IMPORTEDDATE;
...
CASE WHEN LSQ_TESTRIDE.RelatedProspectID IS NOT NULL THEN 'Yes' else 'No' END AS IsTestRideTaken,
  ...
  CASE WHEN UPPER(LSQ_PBASE.ProspectStage) IN ('BOOKED','ALLOCATED') THEN 'Booked'
    ...
    LATEST_FOLLOWUP.LatestIsCustomerContacted,
			...
			END AS FollowupBucket,  -- Please refer the email Busssines logic from arif 09/25/2025
     NULL as LeadLostSecondaryReason ,
...
SELECT @TargetCount1 = COUNT( PK_EnquiryHeaderID) FROM ASM_PB_HK_ENQUIRY_DIM where EnquiryDate= Cast(Getdate()-1 as date)
...
SELECT @TargetCount2 =  COUNT( fK_EnquiryDocID)  FROM ASM_PB_HK_ENQUIRY_FACT_LSQ_STG where Date= Cast(Getdate()-1 as date)
