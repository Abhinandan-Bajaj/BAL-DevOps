...
/*	                                            ASM_CV_SERVICE_COMPLAINT_MASTER_DIM updated code */
/*	2024-10-03 	|	Aniket Mahulikar    ASM_CV_SERVICE_SPARE_CLAIM_HEADER_DIM INC updated code */
/*	                                            ASM_CV_SERVICE_COMPLAINT_MASTER_DIM INC updated code */  
/*  2024-11-28  |   Aniket Mahulikar    Updated logic of automated and manual ffr as shared by BU and Parent and child in spare dim*/
/* 2024-11-28  |   Aniket Mahulikar  Updated INC Load Script                                        */
/*--------------------------------------------------------------------------------------------------*/
/*******************************************HISTORY**************************************************/
...
CREATE PROC [dbo].[USP_ASM_CV_SERVICE_DIMENSION_INCLOAD_P3] AS

BEGIN
declare @ASM_CV_SPARE_CLAIM_loaddate date;
set @ASM_CV_SPARE_CLAIM_loaddate = CAST((SELECT MAX(IMPORTEDDATE) FROM ASM_CV_SERVICE_SPARE_CLAIM_HEADER_DIM)AS DATE);
...
DELETE FROM ASM_CV_SERVICE_SPARE_CLAIM_HEADER_DIM WHERE Creation_Date > Cast(DATEADD(mi,30,(DATEADD(hh,5,getdate()))) - 1 as date);
...
left join #ChildFFRMapping B ON B.FFRHeaderID = SCH.FFR_ParentID WHERE SCH.FFR_Identification = 'Child'
...
(                  
  ...
  FROM ASM_CV_SERVICE_SPARE_CLAIM_HEADER_DIM              
)          
DELETE FROM CTE                  
WHERE RNK<>1;
----------------------------------------------------------------------------------
...
DECLARE @MAXDATESTG01 DATETIME2(7)= (SELECT CAST(MAX(IMPORTEDDATE) AS DATE) FROM ASM_CV_SERVICE_COMPLAINT_MASTER_DIM)
...
AND FORMAT(CAST(CR.ImportedDate AS DATE),'yyyy-MM-dd') > @MAXDATESTG01
...
(                  
  SELECT *,                  
   DENSE_RANK()OVER(PARTITION BY IsrDocid ORDER BY IMPORTEDDATE DESC)RNK                  
  FROM ASM_CV_SERVICE_COMPLAINT_MASTER_DIM 
)          
DELETE FROM CTE                  
WHERE RNK<>1;
----------------------------------------------------------------------------------

PRINT('ASM_CV_SERVICE_COMPLAINT_MASTER_DIM  TABLE LOADED')
END
GO