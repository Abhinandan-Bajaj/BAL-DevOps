...
ALter PROC [dbo].[USP_Full_Load_ASM_PB_T_TEST_RIDE_REFRESH] AS
BEGIN
...
/*******************************************HISTORY**************************************************/
------------------------------------------------------------------------------------------------------------------------------------------
TRUNCATE TABLE ASM_PB_T_TEST_RIDE_FACT;
...
SELECT
 ...
 ----------------------------------------------------------------------------------------------------------------------------------------------
	...
	and LSQ_PEXTBASE.mx_BU_sub_type='TRM'
    AND LSQ_PEXTBASE.mx_Dealer_Assignment_Date  IS NOT NULL
    AND CAST( LSQ_TESTRIDE.CREATEDON AS DATE)  between '2023-04-01' and '2025-06-08'
  ) TMP
	GROUP BY 
DEALERCODE, SKU,FK_DEALERCODE, FK_SKU,  FK_TYPE_ID,DATE,ENQUIRYLINEID,
 FK_ENQUIRYDOCID,COMPANYTYPE,BRANCHID,LASTUPDATEDDATETIME,IMPORTEDDATE,CDMS_BATCHNO ,TARGETQUANTITY,PERIODNAME
...
SELECT
 ...
 ----------------------------------------------------------------------------------------------------------------------------------------------
	...
	and LSQ_PEXTBASE.mx_BU_sub_type='TRM'
    AND LSQ_PEXTBASE.mx_Dealer_Assignment_Date  IS NOT NULL
    AND CAST( LSQ_PEXTBASE.mx_Enquiry_update_date_and_time as date)  between  '2023-04-01' and '2025-06-08'
  ) TMP
	GROUP BY 
DEALERCODE, SKU,FK_DEALERCODE, FK_SKU,  FK_TYPE_ID,DATE,ENQUIRYLINEID,
 FK_ENQUIRYDOCID,COMPANYTYPE,BRANCHID,LASTUPDATEDDATETIME,IMPORTEDDATE,CDMS_BATCHNO ,TARGETQUANTITY,PERIODNAME
