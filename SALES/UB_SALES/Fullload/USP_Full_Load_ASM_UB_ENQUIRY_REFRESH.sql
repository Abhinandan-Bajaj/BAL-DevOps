...
ALTER PROC [dbo].[USP_Full_Load_ASM_UB_ENQUIRY_REFRESH] AS
BEGIN
--***************************START************************************************
/*******************************************HISTORY**************************************************/
/*--------------------------------------------------------------------------------------------------*/
...
AND LATEST_FOLLOWUP.LatestFollowupDate>EXTUB.mx_dealer_assignment_Date
 ...
 and CAST (EXTUB.mx_Dealer_Assignment_Date AS dATE) BETWEEN '2021-04-01' AND Cast(Getdate()-1 as date)
 --AND UB.Modifiedon > (SELECT MAX(IMPORTEDDATE) FROM ASM_UB_ENQUIRY_DIM)
...
where EXTUB.mx_Dealer_Assignment_Date is not null 
 ...
 --AND UB.Modifiedon > (SELECT MAX(IMPORTEDDATE) FROM ASM_UB_ENQUIRY_STG)
