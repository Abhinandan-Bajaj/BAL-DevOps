...
GO
alter PROC [dbo].[USP_ASM_PB_HKT_TEST_RIDE_REFRESH] AS
BEGIN
...
DECLARE @SPID INT = @@SPID,
        @sp_name VARCHAR(128) = 'USP_ASM_PB_HKT_TEST_RIDE_REFRESH';
...
DECLARE @StartDate_utc1 DATETIME = GETDATE(),
			...
			@table_name1 VARCHAR(128) = 'ASM_PB_HK_TEST_RIDE_FACT', 
            @SourceCount1 BIGINT,  
            @TargetCount1 BIGINT,   
            @Status1 VARCHAR(10),
...
WHERE LSQ_PACTEXTBASE.ActivityEvent=12002 
	--and LSQ_PEXTBASE.mx_BU_sub_type='TRM'
    AND LSQ_PEXTBASE.mx_Dealer_Assignment_Date  IS NOT NULL
    AND CAST( FIRST_FOLLOWUP.FirstfollowupDate AS DATE)   >=@ASMFollowp_IMPORTEDDATE
  ) TMP
	GROUP BY 
DEALERCODE, SKU,FK_DEALERCODE, FK_SKU,  FK_TYPE_ID,DATE,ENQUIRYLINEID,
 FK_ENQUIRYDOCID,COMPANYTYPE,BRANCHID,LASTUPDATEDDATETIME,IMPORTEDDATE,CDMS_BATCHNO ,TARGETQUANTITY,PERIODNAME
...
SELECT
 ...
 ----------------------------------------------------------------------------------------------------------------------------------------------
	...
	--and LSQ_PEXTBASE.mx_BU_sub_type='TRM'
    AND LSQ_PEXTBASE.mx_Dealer_Assignment_Date  IS NOT NULL
    AND CAST( LSQ_PEXTBASE.mx_Enquiry_update_date_and_time AS DATE)   >=@ASMFact_IMPORTEDDATE
  ) TMP
	GROUP BY 
DEALERCODE, SKU,FK_DEALERCODE, FK_SKU,  FK_TYPE_ID,DATE,ENQUIRYLINEID,
 FK_ENQUIRYDOCID,COMPANYTYPE,BRANCHID,LASTUPDATEDDATETIME,IMPORTEDDATE,CDMS_BATCHNO ,TARGETQUANTITY,PERIODNAME
