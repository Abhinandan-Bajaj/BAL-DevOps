--with CXP changes and first_source_changes 
/****** Object:  StoredProcedure [dbo].[USP_ASM_MC_ENQUIRY_REFRESH]    Script Date: 7/18/2024 6:26:20 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROC [dbo].[USP_ASM_MC_ENQUIRY_REFRESH] AS
BEGIN
--***************************START************************************************
/*******************************************HISTORY**************************************************/
/*--------------------------------------------------------------------------------------------------*/
...
declare @ASM_MC_ENQUIRY_DIM_CDMS_STG_loaddate date;
set @ASM_MC_ENQUIRY_DIM_CDMS_STG_loaddate=CAST((SELECT MAX(IMPORTEDDATE) FROM ASM_MC_ENQUIRY_DIM_CDMS_STG)AS DATE);
...
FROM
  ...
  INNER JOIN COMPANY_MASTER ON (ENQUIRY_HEADER.COMPANYID=COMPANY_MASTER.COMPANYID AND COMPANY_MASTER.COMPANYTYPE IN (1,8))
				...
				--WHERE CAST(ENQUIRY_HEADER.IMPORTEDDATE AS DATE) >= CAST((SELECT MAX(IMPORTEDDATE) FROM ASM_MC_ENQUIRY_DIM_CDMS_STG)AS DATE)  -- UPDATED ON 12/06
				--WHERE CAST(ENQUIRY_HEADER.DOCDATE AS DATE) BETWEEN '2020-04-01' AND Cast(Getdate()-1 as date) 
				WHERE CAST(ENQUIRY_HEADER.IMPORTEDDATE AS DATE) >= @ASM_MC_ENQUIRY_DIM_CDMS_STG_loaddate
...
--this Deduplication should always be performed on Delta (data for that day) or Full data ?
	Delete from ASM_MC_ENQUIRY_DIM_CDMS_STG Where Cast(EnquiryDate as date)>Cast(DATEADD(mi,30,(DATEADD(hh,5,getdate()))) - 1 as date)
...
declare @ASM_MC_ENQUIRY_DIM_LSQ_STG_loaddate date;
set @ASM_MC_ENQUIRY_DIM_LSQ_STG_loaddate=CAST((SELECT MAX(IMPORTEDDATE) FROM ASM_MC_ENQUIRY_DIM_LSQ_STG) AS DATE);
...
-------TestRIDE Logic------------------------------------------------------
 ...
 ON LSQ_PEXT2BASE.ProspectId=LSQ_PBASE.PROSPECTID
  ...
  AND  LSQ_PBASE.mx_BU='MC'
    ...
    --AND CAST (DATEADD(mi,30,(DATEADD(hh,5,LSQ_PBASE.ModifiedOn))) AS DATE )  BETWEEN '2020-04-01' AND Cast(Getdate()-1 as date) 
	--AND DATEADD(mi,30,(DATEADD(hh,5,LSQ_PBASE.ModifiedOn))) > (SELECT MAX(IMPORTEDDATE) FROM ASM_MC_ENQUIRY_DIM_LSQ_STG)
	--AND DATEADD(mi,30,(DATEADD(hh,5,LSQ_PBASE.ModifiedOn)))  >= CAST((SELECT MAX(IMPORTEDDATE) FROM ASM_MC_ENQUIRY_DIM_LSQ_STG) AS DATE)  -- UPDATED ON 12/06
	AND DATEADD(mi,30,(DATEADD(hh,5,LSQ_PBASE.ModifiedOn)))  >= @ASM_MC_ENQUIRY_DIM_LSQ_STG_loaddate

------------------------------------------------------------------------------------------------------------------- 
	Delete from ASM_MC_ENQUIRY_DIM_LSQ_STG Where Cast(EnquiryDate as date)>Cast(DATEADD(mi,30,(DATEADD(hh,5,getdate()))) - 1 as date)
...
------ e. Product Master and Dealer Master FK update: ASM_MC_ENQUIRY_TARGET  */
	declare @ASM_MC_ENQUIRY_STG_loaddate date;
	set @ASM_MC_ENQUIRY_STG_loaddate=CAST((SELECT MAX(IMPORTEDDATE) FROM ASM_MC_ENQUIRY_STG) AS DATE);
...
FROM 
   ...
   LEFT JOIN ITEMVARMATRIX_MASTER IV ON (EL.VARMATRIXID=IV.ITEMVARMATRIXID)
	   ...
	   -- WHERE CAST(EH.IMPORTEDDATE AS DATE) >= CAST((SELECT MAX(IMPORTEDDATE) FROM ASM_MC_ENQUIRY_STG) AS DATE)   -- UPDATED ON 12/06
	 --WHERE CAST(EH.DOCDATE AS DATE) BETWEEN '2020-04-01' AND Cast(Getdate()-1 as date) 
	 WHERE CAST(EH.IMPORTEDDATE AS DATE) >= @ASM_MC_ENQUIRY_STG_loaddate
	   )base
...
,COLOUR_CODE, MODELCODE, FK_MODEL,FLAG, BaseFlag, TEHSILID,SALESPERSON,LEADTYPE,First_Source_Lead_Type, First_Mode_Source,First_Mode_SubSource
   ...
   --
	...
	Delete from ASM_MC_ENQUIRY_STG Where DATE> Cast(DATEADD(mi,30,(DATEADD(hh,5,getdate()))) - 1 as date)
...
--------------------------------------------------------------------------------------------------------------------------------------- */
	...
	---------------------------STEP 2.2 LOAD LSQ Data into Enquiry Fact LSQ transactions Data----------------------------------------------
	--------------------------------------------------------------------------------------------------------------------------------------- */
	declare @ASM_MC_ENQUIRY_FACT_LSQ_STG_loaddate date;
	set @ASM_MC_ENQUIRY_FACT_LSQ_STG_loaddate=CAST((SELECT MAX(IMPORTEDDATE) FROM ASM_MC_ENQUIRY_FACT_LSQ_STG) AS DATE);
   ...
   LSQ_PEXT2BASE.ProspectId =LSQ_PBASE.ProspectId
    ...
    AND LSQ_PEXTBASE.mx_Dealer_Assignment_Date is not null
			...
			--AND DATEADD(mi,30,(DATEADD(hh,5,LSQ_PBASE.ModifiedOn))) >       (SELECT MAX(IMPORTEDDATE) FROM ASM_MC_ENQUIRY_FACT_LSQ_STG)
			 -- AND DATEADD(mi,30,(DATEADD(hh,5,LSQ_PBASE.ModifiedOn))) >= CAST((SELECT MAX(IMPORTEDDATE) FROM ASM_MC_ENQUIRY_FACT_LSQ_STG) AS DATE)  -- UPDATED ON 12/06
			 AND DATEADD(mi,30,(DATEADD(hh,5,LSQ_PBASE.ModifiedOn))) >= @ASM_MC_ENQUIRY_FACT_LSQ_STG_loaddate
...
ON LSQ_STG.FK_ENQUIRYDOCID =EZSDD.Enquiry
	...
	Delete from ASM_MC_ENQUIRY_FACT_LSQ_STG Where DATE>Cast(DATEADD(mi,30,(DATEADD(hh,5,getdate()))) - 1 as date);
	--Dedup Process:
	WITH CTE AS                  
	 (                  
	  SELECT *,                  
	...
	declare @ASM_MC_ENQTORET_CONVERSION_loaddate datetime;
	set @ASM_MC_ENQTORET_CONVERSION_loaddate=(SELECT MAX(DATE) from ASM_MC_ENQTORET_CONVERSION)
	...
	WHERE 
	--[DATE] BETWEEN '2020-04-01' AND Cast(Getdate()-1 as date)
	--[DATE]>(SELECT MAX(DATE) from ASM_MC_ENQTORET_CONVERSION)
	[DATE]>@ASM_MC_ENQTORET_CONVERSION_loaddate
