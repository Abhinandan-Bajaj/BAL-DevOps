...
ALTER PROC [dbo].[USP_ASM_PB_SERVICE_SERVICE_FULLLOAD] AS 
/*******************************************HISTORY**************************************************/
/*--------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------*/
...
/*	2024-07-11 	|	Sarvesh Kulkarni		| Changes for TAT and Triumph Revenue */
...
Begin
print('Truncating table ASM_PB_SERVICE_STG')
TRUNCATE TABLE ASM_PB_SERVICE_STG
print('Inserting data in ASM_PB_SERVICE_STG table')
...
,CASE 
        WHEN MONTH(SH.Docdate) >= 4 THEN Concat('FY ',Cast(YEAR(SH.Docdate)+1 as varchar))
        ELSE Concat('FY ',Cast(YEAR(SH.Docdate) as varchar))
END service_fiscal_year
,SLE.DefectCode AS DefectCode
,'Others' AS PartRepairType
,CASE
...
WHERE CAST(SH.DOCDATE AS DATE) BETWEEN '2018-04-01' AND '2020-03-31'
...
,CASE    WHEN ISNULL(IBM.INVOICEDATE, '') = '' THEN NULL
        ...
        ELSE Concat('FY ',Cast(YEAR(IBM.INVOICEDATE) as varchar))
END retail_fiscal_year
,CASE WHEN MONTH(SH.Docdate) >= 4 THEN Concat('FY ',Cast(YEAR(SH.Docdate)+1 as varchar))
        ELSE Concat('FY ',Cast(YEAR(SH.Docdate) as varchar))
END service_fiscal_year
,SLE.DefectCode AS DefectCode
,'Others' AS PartRepairType
...
1=1
--SH.ISCLOSED=0
AND CAST(SH.DOCDATE AS DATE) >='2018-04-01'
AND SH.CANCELLATIONDATE IS NULL
