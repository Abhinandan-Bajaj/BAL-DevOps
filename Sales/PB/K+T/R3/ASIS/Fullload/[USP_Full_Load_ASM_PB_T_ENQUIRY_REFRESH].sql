...
GO
ALTER PROC [dbo].[USP_Full_Load_ASM_PB_T_ENQUIRY_REFRESH] AS
BEGIN
/*******************************************HISTORY**************************************************/
/*--------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------------------------------*/
/*	2024-06-21 	|	Lachmanna L		| Added sales person Enquire Fact Table                       			*/
/*	2024-09-30 	|	Nikita L		| First Source Lead Type column addition                    			
    2024-12-09	|	Richa		     | Lost Enquiry Columns                
...
TRUNCATE TABLE ASM_PB_T_ENQTORET_CONVERSION
...
AND LATEST_FOLLOWUP.LatestFollowupDate> LSQ_PEXTBASE.mx_dealer_assignment_Date
 ...
 ON LSQ_PEXT2BASE.ProspectId=LSQ_PBASE.PROSPECTID
  ...
  WHERE LSQ_PEXTBASE.mx_Dealer_Assignment_Date is not null
	and LSQ_PEXTBASE.mx_BU_sub_type='TRM'
	AND CAST(LSQ_PBASE.ModifiedOn AS DATE) BETWEEN '2022-04-01' AND '2025-06-08'  
  --AND LSQ_PBASE.ModifiedOn > (SELECT  MAX(IMPORTEDDATE) FROM ASM_TRIUMPH_ENQUIRY_DIM); --- 12002 for MC
...
SELECT
  ...
  LSQ_PBASE.ProspectID as FK_ENQUIRYDOCID, 
   ...
   FROM
	...
	AND CAST(LSQ_PBASE.ModifiedOn AS DATE) BETWEEN '2025-06-09' AND Cast(Getdate()-1 as date)
	--AND LSQ_PBASE.modifiedon > (SELECT MAX(IMPORTEDDATE) FROM ASM_PB_T_ENQUIRY_STG)
  ) TMP
	GROUP BY 
...
SELECT
  ...
  LSQ_PBASE.ProspectID as FK_ENQUIRYDOCID, 
   ...
   FROM
    ...
    AND LSQ_PEXTBASE.mx_Dealer_Assignment_Date is not null
	AND CAST(LSQ_PBASE.ModifiedOn AS DATE) BETWEEN '2022-04-01' AND '2025-06-08'
	--AND LSQ_PBASE.modifiedon > (SELECT MAX(IMPORTEDDATE) FROM ASM_TRIUMPH_ENQUIRY_STG)
  ) TMP
	GROUP BY 
DEALERCODE, SKU,FK_DEALERCODE, FK_SKU,  FK_TYPE_ID,DATE,ENQUIRYLINEID,
 FK_ENQUIRYDOCID,COMPANYTYPE,BRANCHID,LASTUPDATEDDATETIME,IMPORTEDDATE,CDMS_BATCHNO ,TARGETQUANTITY,PERIODNAME
...
EF.IMPORTEDDATE,
GETDATE() AS CREATEDDATETIME
--INTO ASM_PB_T_ENQTORET_CONVERSION
FROM 
...
--[DATE]>(SELECT MAX(DATE) from ASM_PB_T_ENQTORET_CONVERSION)
