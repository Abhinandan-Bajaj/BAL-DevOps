...
GO
ALTER PROC [dbo].[USP_ASM_PB_HKT_RETAIL_REFRESH] AS
BEGIN
/*******************************************HISTORY**************************************************/
/*--------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------*/
...
set @ASMDim_IMPORTEDDATE = CAST((SELECT MAX(IMPORTEDDATE) FROM ASM_PB_HK_RETAIL_DIM)AS DATE);

declare @ASMFact_IMPORTEDDATE date;
set @ASMFact_IMPORTEDDATE = CAST((SELECT MAX(IMPORTEDDATE) FROM ASM_PB_HK_RETAIL_STG)AS DATE);


DECLARE @SPID INT = @@SPID,
        @sp_name VARCHAR(128) = 'USP_ASM_PB_HKT_RETAIL_REFRESH';
...
DECLARE @StartDate_utc1 DATETIME = GETDATE(),
			...
			@table_name1 VARCHAR(128) = 'ASM_PB_HK_RETAIL_DIM', 
            @SourceCount1 BIGINT,  
            @TargetCount1 BIGINT,   
            @Status1 VARCHAR(10),
...
DECLARE @StartDate_utc2 DATETIME = GETDATE(),
		...
		@table_name2 VARCHAR(128) = 'ASM_PB_HK_RETAIL_STG', 
        @SourceCount2 BIGINT,  
        @TargetCount2 BIGINT,   
        @Status2 VARCHAR(10),
...
EXEC [USP_ASM_PB_T_RETAIL_REFRESH];
