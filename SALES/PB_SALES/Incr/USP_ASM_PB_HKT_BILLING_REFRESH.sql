...
alter  PROC [dbo].[USP_ASM_PB_HKT_BILLING_REFRESH] AS
BEGIN
--***************************START************************************************
/*******************************************HISTORY**************************************************/
/*--------------------------------------------------------------------------------------------------*/
...
set @ASMFact_IMPORTEDDATE = CAST((SELECT MAX(DATE) from ASM_PB_HK_BILLING_STG)AS DATE);


DECLARE @SPID INT = @@SPID,
        @sp_name VARCHAR(128) = 'USP_ASM_PB_HKT_BILLING_REFRESH';
...
DECLARE @StartDate_utc1 DATETIME = GETDATE(),
			...
			@table_name1 VARCHAR(128) = 'ASM_PB_HK_BILLING_STG', 
            @SourceCount1 BIGINT,  
            @TargetCount1 BIGINT,   
            @Status1 VARCHAR(10),
...
INSERT INTO ASM_PB_HK_BILLING_STG
SELECT DISTINCT
...
FROM
   SAP_BILLING 
INNER JOIN COMPANY_MASTER CM ON ('0000'+SAP_BILLING.DEALERCODE=CM.Code AND(CM.COMPANYTYPE=2 ))
INNER JOIN (
    SELECT *, 
...
TRUNCATE TABLE ASM_PB_HK_BILLING_PLAN_STG

INSERT INTO ASM_PB_HK_BILLING_PLAN_STG
SELECT DISTINCT
ZO.DEALERCODE,
ZO.MATERIAL as SKU,
Cast(0 as int) as FK_DEALERCODE,
...
INNER JOIN (
    ...
    ON 
    PM.MODELCODE = LEFT(ZO.MATERIAL,6)
    AND PM.rnk = 1
WHERE 
ZO.DEALERCODE IN (SELECT DISTINCT KUNNR FROM SAP_ZSD_DEALER_REPOS WHERE KATR6='PB')
AND ZO.DEALERCODE IN (SELECT DISTINCT CODE FROM COMPANY_MASTER WHERE COMPANYTYPE=2 )
...
NULL AS TEHSILID,
NULL AS SALESPERSON
--INTO ASM_PB_BILLING_PLAN_STG
FROM 
SAP_ZMPSS_ORDERS ZO
WHERE 
ZO.DEALERCODE IN (SELECT DISTINCT KUNNR FROM SAP_ZSD_DEALER_REPOS WHERE KATR6='PB')
AND ZO.DEALERCODE IN (SELECT DISTINCT CODE FROM COMPANY_MASTER WHERE COMPANYTYPE=2 AND COMPANYSUBTYPE IS NULL)
AND  CAST(ZO.DOCDATE AS DATE) BETWEEN '2025-06-09' AND  '2025-06-08' 
Group By
	ZO.DEALERCODE,
	ZO.MATERIAL,
	ZO.DOCDATE,
...
TRUNCATE TABLE ASM_PB_HK_PENDING_ORDER_STG

INSERT INTO ASM_PB_HK_PENDING_ORDER_STG
SELECT DISTINCT
...
INNER JOIN (
    ...
    ON 
    PM.MODELCODE = LEFT(PO.MATERIAL,6)
    AND PM.rnk = 1
WHERE 
'00000'+PO.SOLDTOPARTY IN (SELECT DISTINCT KUNNR from SAP_ZSD_DEALER_REPOS Where KATR6='PB') 
AND '00000'+PO.SOLDTOPARTY IN (SELECT DISTINCT CODE FROM COMPANY_MASTER WHERE COMPANYTYPE=2 )
...
WHERE 
'00000'+PO.SOLDTOPARTY IN (SELECT DISTINCT KUNNR from SAP_ZSD_DEALER_REPOS Where KATR6='PB') 
AND '00000'+PO.SOLDTOPARTY IN (SELECT DISTINCT CODE FROM COMPANY_MASTER WHERE COMPANYTYPE=2 AND COMPANYSUBTYPE IS NULL)
AND FUND='F'  
...
--**********************************************************************
--4.Billing Fact
TRUNCATE TABLE ASM_PB_HK_BILLING_FACT
INSERT INTO ASM_PB_HK_BILLING_FACT
SELECT * FROM ASM_PB_HK_BILLING_STG
UNION
SELECT *,NULL AS BRANCHCODE FROM ASM_PB_HK_BILLING_PLAN_STG
UNION 
SELECT *,NULL AS BRANCHCODE FROM ASM_PB_HK_PENDING_ORDER_STG

--Product Master and Dealer Master FK update: ASM_MC_BILLING_FACT
update B set B.FK_SKU=C.PK_SKU from ASM_PB_HK_BILLING_FACT B INNER JOIN ASM_PB_HKT_PRODUCT_SKU_DIM C on (B.SKU=C.SKU_CODE)
update B set B.FK_MODEL=C.PK_Model_Code from ASM_PB_HK_BILLING_FACT B INNER JOIN ASM_PB_HKT_PRODUCT_DIM C on (B.MODELCODE=C.MODELCODE)
update B set B.FK_DEALERCODE=C.PK_DEALERCODE from [dbo].ASM_PB_HK_BILLING_FACT B INNER JOIN ASM_PB_HKT_DEALER_MASTER_DIM C on (B.DEALERCODE=C.DEALERCODE)
--******************************************************************************

EXEC [USP_ASM_PB_T_BILLING_REFRESH];
