...
Alter  PROC [dbo].[USP_ASM_PB_SERVICE_DIMENSION_INCLOAD_P3]
AS
BEGIN
/*******************************************HISTORY**************************************************/
/*--------------------------------------------------------------------------------------------------*/
...
/*2025-05-15      |   Richa               | Additional columns in Compaint master Dim*/
/*--------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------*/
/*******************************************HISTORY**************************************************/
...
declare @ASM_PB_SPARE_CLAIM_loaddate date;
SET @ASM_PB_SPARE_CLAIM_loaddate  = CAST((SELECT MAX(IMPORTEDDATE) FROM ASM_PB_SERVICE_SPARE_CLAIM_HEADER_DIM) AS DATE);


--INC LOAD
--1. Load Spare Claim Header DIM for PB
...
AND CAST(SCH.ImportedDate AS DATE) >= @ASM_PB_SPARE_CLAIM_loaddate 

Delete from ASM_PB_SERVICE_SPARE_CLAIM_HEADER_DIM Where Creation_Date >Cast(DATEADD(mi,30,(DATEADD(hh,5,getdate()))) - 1 as date);
