...
CREATE PROC [dbo].[USP_ASM_SPARES_ICMC_BACKORDER_INCLOAD]
AS
BEGIN
/*******************************************HISTORY**************************************************/
/*--------------------------------------------------------------------------------------------------*/
...
/*  2025-02-24  |       Aswani    |        Included the partial orders logic                        */
...
-------------------------------------------------------------**PRIMARY SCREEN BACKORDER DATA LOAD**----------------------------------------------------------
...
AND REFRESH_DATE BETWEEN @StartDate AND @CurrentDate
...
AND REFRESH_DATE = CAST(GETDATE() AS DATE)
)A
...
----------------------------------------BO_DAY1_OF_MONTH_DATA_LOAD----------------------------------------------

WITH MAX_DATE AS 
(
SELECT MAX(CALENDAR_DATE) AS CALENDAR_DATE_MAX,
MAX(SOLD_TO_DEALER_OR_DISTRIBUTOR) AS SOLD_TO_DEALER_OR_DISTRIBUTOR,
MAX(BU) AS BU,
MAX(BACKORDER_AMOUNT) AS BACKORDER_AMOUNT
...
SELECT 
CALENDAR_DATE_MAX AS CALENDAR_DATE,
SOLD_TO_DEALER_OR_DISTRIBUTOR,
BU,
BACKORDER_AMOUNT,
CAST(getdate() AS DATE) AS CREATE_DATE
...
PRINT('Backorder data loaded successfully')
