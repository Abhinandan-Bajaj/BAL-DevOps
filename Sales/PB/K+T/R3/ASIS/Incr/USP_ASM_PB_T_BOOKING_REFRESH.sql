...
GO
Create PROC [dbo].[USP_ASM_PB_T_BOOKING_REFRESH] AS
BEGIN
--***************************START*****************************
/*******************************************HISTORY**************************************************/
/*--------------------------------------------------------------------------------------------------*/
...
AND Cast(BOOKING_HEADER.IMPORTEDDATE as date)>=(SELECT MAX(IMPORTEDDATE) FROM ASM_PB_T_BOOKING_DIM)
...
BH.DOCTYPE=1000050
AND CAST(BH.IMPORTEDDATE as date) >=(SELECT MAX(IMPORTEDDATE) FROM ASM_PB_T_BOOKING_FACT)
GROUP BY
CM.CODE,
IM.CODE+IV.CODE,
