...
/*   2025-03-05	|	Richa Mishra		| Added Material Dim for Spares TRM Bifurcation  */
/*	2025-03-12 	|	Dewang Makani		| Added SAP ProductionDate column in install base master */
/*	2025-04-10 	|	Dewang Makani		| Added Modelid and Engine No column in install base master table*/
/*	2025-09-30 	|	Rashi Pradhan		| Audit log for install base master*/
/*                                                                                               */
/*--------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------*/
/*******************************************HISTORY**************************************************/
...
DECLARE @SPID INT = @@SPID,
        @sp_name VARCHAR(128) = 'USP_ASM_SERVICE_MASTER_REFRESH';

----------------------------------------------------------------
    -- Audit Segment 1: ASM_SERVICE_INSTALLBASE_MASTER_DIM
----------------------------------------------------------------  
...
DECLARE @StartDate_utc1 DATETIME = GETDATE(),
			...
			@table_name1 VARCHAR(128) = 'ASM_SERVICE_INSTALLBASE_MASTER_DIM', 
            @SourceCount1 BIGINT,  
            @TargetCount1 BIGINT,   
            @Status1 VARCHAR(10),
...
----------------------------------Audit Log Source
    ...
    BEGIN TRY
        SELECT @SourceCount1 = COUNT(DISTINCT IBM.IBID)
		FROM INSTALL_BASE_MASTER IBM
	    LEFT JOIN SAP_YMFGT_CHASSIS_VODATE SAP_IBM ON SAP_IBM.CHASSIS = IBM.NAME
...
select DISTINCT IBID
	  ...
	  ,CAST(SAP_IBM.VODATE AS DATE) AS SAP_ProductionDate 
      ,IBM.Itemid AS ModelID
	  ,IBM.ComponentNo AS EngineNo
...
SELECT @TargetCount1 = COUNT(DISTINCT PK_Ibid) FROM ASM_SERVICE_INSTALLBASE_MASTER_DIM;
...
-----------------------------******** Material Description for TRM &BB Bifurcation ****--------------------------------

Truncate table ASM_SERVICE_MATERIAL_DIM

INSERT INTO ASM_SERVICE_MATERIAL_DIM(
[ItemID]
,[ItemCode]
,[Material]
,[Refresh_Date]
