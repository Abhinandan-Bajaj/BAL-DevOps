/****** Object:  StoredProcedure [dbo].[USP_ASM_PB_HKT_BOOKING_REFRESH]    Script Date: 7/21/2025 6:41:30 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROC [dbo].[USP_ASM_PB_HKT_BOOKING_REFRESH] AS
BEGIN
/*******************************************HISTORY**************************************************/
/*--------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------*/
...
set @ASMDim_IMPORTEDDATE = CAST((SELECT MAX(IMPORTEDDATE) FROM ASM_PB_HK_BOOKING_DIM)AS DATE);

declare @ASMFact_IMPORTEDDATE date;
set @ASMFact_IMPORTEDDATE = CAST((SELECT MAX(IMPORTEDDATE) FROM ASM_PB_HK_BOOKING_FACT)AS DATE);


DECLARE @SPID INT = @@SPID,
        @sp_name VARCHAR(128) = 'USP_ASM_PB_HKT_BOOKING_REFRESH';
...
--1. Booking Dim:
INSERT INTO ASM_PB_HK_BOOKING_DIM
SELECT DISTINCT
...
FROM BOOKING_HEADER 
INNER JOIN BOOKING_LINE BL ON (BOOKING_HEADER.HEADERID=BL.HEADERID)
JOIN ITEM_MASTER IM ON (IM.ItemId=BL.ItemID)
INNER JOIN COMPANY_MASTER ON (BOOKING_HEADER.COMPANYID=COMPANY_MASTER.COMPANYID 
AND (COMPANY_MASTER.COMPANYTYPE = 2 ))-- AND COMPANY_MASTER.COMPANYSUBTYPE  is null))
...
;WITH CTE AS                  
 (                  
  ...
  FROM ASM_PB_HK_BOOKING_DIM                  
 )                  
DELETE FROM CTE                  
 WHERE RNK<>1;
--*************************************************************************
--2. Booking Fact:
INSERT INTO ASM_PB_HK_BOOKING_FACT
SELECT
DISTINCT 
...
NULL AS TEHSILID,
NULL AS SALESPERSON,
ISNULL(COALESCE(EH.LEADTYPE,LSQ_PBASE.mx_Enquiry_Mode, LSQ_PBASE.MX_MODE_OF_ENQUIRY),'Not Available') as LeadType,
...
DELETE FROM ASM_PB_HK_BOOKING_FACT WHERE DATE>Cast(Getdate()-1 as date)
...
update B set B.FK_SKU=C.PK_SKU from ASM_PB_HK_BOOKING_FACT B INNER JOIN ASM_PB_HKT_PRODUCT_SKU_DIM C on (B.SKU=C.SKU_CODE)
update B set B.FK_MODEL=C.PK_Model_Code from ASM_PB_HK_BOOKING_FACT B INNER JOIN ASM_PB_HKT_PRODUCT_DIM C on (B.MODEL=C.MODELCODE)
update B set B.FK_DEALERCODE=C.PK_DEALERCODE from [dbo].ASM_PB_HK_BOOKING_FACT B INNER JOIN ASM_PB_HKT_DEALER_MASTER_DIM C on (B.DEALERCODE=C.DEALERCODE)

--******************************************************************************************************************


EXEC [USP_ASM_PB_T_BOOKING_REFRESH];

TRUNCATE TABLE ASM_PB_HKT_BOOKING_DIM
insert into ASM_PB_HKT_BOOKING_DIM
select *,'PB-KTM' from ASM_PB_HK_BOOKING_DIM
insert into ASM_PB_HKT_BOOKING_DIM
select *,'PB-TRM' from ASM_PB_T_BOOKING_DIM
