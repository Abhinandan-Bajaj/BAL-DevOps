...
CREATE PROC [dbo].[USP_ASM_PB_SERVICE_APP_INCLOAD]
AS
BEGIN
/*******************************************HISTORY**************************************************/
/*--------------------------------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------------------------------*/
/*	2025-01-03 	|	Richa	  | New INCLOAD SP for Service App Dashboard     */              
/*--------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------*/
/*******************************************HISTORY**************************************************/


DECLARE @MAXDATESTG02 DATETIME2(7)= (SELECT MAX(IMPORTEDDATE) FROM ASM_PB_SERVICE_ORDER_DIM)

DECLARE @MAXDATESTG03 DATETIME2(7)= (SELECT MAX(IMPORTEDDATE) FROM ASM_PB_SERVICE_APPOINTMENT_DIM)
...
where  Cast(STG.Importeddate as Date) >= Cast(@MAXDATESTG02 as date)
...
---------------De Dup-----------------------------------
 ...
 (                  
  SELECT *,                  
   DENSE_RANK()OVER(PARTITION BY Job_card_name ORDER BY IMPORTEDDATE DESC)RNK                   
  FROM ASM_PB_SERVICE_ORDER_DIM                  
 )                  
DELETE FROM CTE                  
 WHERE RNK<>1;  

print('Dedup of ASM_PB_SERVICE_ORDER_DIM')
--------------APPOINTMENT------------------------
...
WHERE
 cast(AH.ImportedDate as date) >= cast(@MAXDATESTG03 as date)
 and Upper(IM.Name) Not like 'HUSQVARNA%'
...
---------------De Dup-----------------------------------
 ...
 (                  
  SELECT *,                  
   DENSE_RANK()OVER(PARTITION BY DocumentName ORDER BY IMPORTEDDATE DESC)RNK                   
  FROM ASM_PB_SERVICE_APPOINTMENT_DIM                  
 )                  
DELETE FROM CTE                  
 WHERE RNK<>1;  

print('Dedup of ASM_PB_SERVICE_APPOINTMENT_DIM')
END
GO