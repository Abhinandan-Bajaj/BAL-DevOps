...
Alter PROC [dbo].[USP_ASM_PB_HKT_ENQUIRY_REFRESH] AS
BEGIN
/*******************************************HISTORY**************************************************/
/*--------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------*/
...
declare @ASMDimcdms_IMPORTEDDATE date;
set @ASMDimcdms_IMPORTEDDATE = CAST((SELECT MAX(IMPORTEDDATE) FROM ASM_PB_HK_ENQUIRY_DIM_CDMS_STG)AS DATE);

declare @ASMDimlsq_IMPORTEDDATE date;
set @ASMDimlsq_IMPORTEDDATE = CAST((SELECT MAX(IMPORTEDDATE) FROM ASM_PB_HK_ENQUIRY_DIM_LSQ_STG)AS DATE);

declare @ASMStgcdms_IMPORTEDDATE date;
set @ASMStgcdms_IMPORTEDDATE =CAST((SELECT MAX(IMPORTEDDATE) FROM ASM_PB_HK_ENQUIRY_STG) AS DATE);


declare @ASMStglsq_IMPORTEDDATE date;
set @ASMStglsq_IMPORTEDDATE = CAST((SELECT MAX(IMPORTEDDATE) FROM ASM_PB_HK_ENQUIRY_FACT_LSQ_STG)AS DATE);

DECLARE @SPID INT = @@SPID,
        @sp_name VARCHAR(128) = 'SP_ASM_PB_HKT_ENQUIRY_REFRESH';
...
DECLARE @StartDate_utc1 DATETIME = GETDATE(),
			...
			@table_name1 VARCHAR(128) = 'ASM_PB_HK_ENQUIRY_DIM_LSQ_STG', 
            @SourceCount1 BIGINT,  
            @TargetCount1 BIGINT,   
            @Status1 VARCHAR(10),
...
BEGIN TRY
        SELECT @SourceCount1 =  COUNT(distinct LSQ_PBASE.ProspectId)  FROM LSQ_Prospect_Base LSQ_PBASE   
  ...
  AND LSQ_PACTEXTBASE.ActivityEvent=12002 
   ...
   INNER JOIN (select *, Row_number() over (partition by MODELCODE order by MODELCODE) as rnk
	...
	ON  PM.Modelcode = IM.Code and PM.BRAND not in('TRIUMPH') and rnk = 1
...
ENQUIRY_HEADER.IMPORTEDDATE>=@ASMDimcdms_IMPORTEDDATE
...
FROM LSQ_UserTask_Base  WHERE TaskType ='18724219-010f-11ed-8065-02dfc2d2ef6a'
and Try_Cast(CreatedON as DATE)>= ( select  ISNULL(DATEADD(DAY, -2, MAX(EnquiryDate)), '2022-01-01')  from ASM_PB_HK_ENQUIRY_DIM_LSQ_STG)
...
FROM
 ...
 (SELECT DueDate,OwnerId,RelatedEntityId, TaskType,CreatedON, ROW_NUMBER()OVER(PARTITION BY RelatedEntityId ORDER BY CreatedON Asc)RNK                  
  ...
  ON    (LSQ_PBASE.ProspectId = LSQ_UTBASE.RelatedEntityId) 
...
AND DATEADD(mi,30,(DATEADD(hh,5,LATEST_FOLLOWUP.LatestFollowupDate))) > DATEADD(mi,30,(DATEADD(hh,5,LSQ_PEXTBASE.mx_Dealer_Assignment_Date)))
 ...
 -- AND CAST(DATEADD(mi,30,(DATEADD(hh,5,LSQ_PBASE.Modifiedon))) AS DATE) BETWEEN '2025-06-09' AND Cast(Getdate()-1 as date)
  AND DATEADD(mi,30,(DATEADD(hh,5,LSQ_PBASE.ModifiedOn))) >= @ASMDimlsq_IMPORTEDDATE
...
From ASM_PB_HK_ENQUIRY_DIM ED  JOIN  (
...
SELECT @TargetCount1 = COUNT(DISTINCT PK_EnquiryHeaderID) FROM ASM_TRIUMPH_ENQUIRY_DIM;
...
DECLARE @StartDate_utc2 DATETIME = GETDATE(),
		...
		@table_name2 VARCHAR(128) = 'ASM_PB_HK_ENQUIRY_FACT_LSQ_STG', 
        @SourceCount2 BIGINT,  
        @TargetCount2 BIGINT,   
        @Status2 VARCHAR(10),
...
BEGIN TRY
 SELECT @SourceCount2 = COUNT(distinct LSQ_PBASE.ProspectId)  FROM LSQ_Prospect_Base LSQ_PBASE   
  INNER JOIN DBO.BRANCH_MASTER BM 
  ON LSQ_PBASE.mx_Branch_Code=BM.CODE
...
EH.IMPORTEDDATE>=@ASMStgcdms_IMPORTEDDATE
	   )base
...
SELECT
  ...
  LSQ_PBASE.ProspectID as FK_ENQUIRYDOCID, 
   ...
   FROM
	...
	--AND CAST(DATEADD(mi,30,(DATEADD(hh,5,LSQ_PBASE.Modifiedon))) AS DATE) BETWEEN '2025-06-09' AND Cast(Getdate()-1 as date)
  AND DATEADD(mi,30,(DATEADD(hh,5,LSQ_PBASE.Modifiedon))) >=@ASMStglsq_IMPORTEDDATE
  ) TMP
	GROUP BY 
DEALERCODE, SKU,FK_DEALERCODE, FK_SKU,  FK_TYPE_ID,DATE,ENQUIRYLINEID,
 FK_ENQUIRYDOCID,COMPANYTYPE,BRANCHID,LASTUPDATEDDATETIME,IMPORTEDDATE,CDMS_BATCHNO ,TARGETQUANTITY,PERIODNAME
...
--RetailConversion Flag:
 ...
 -- Update RetailConversion Flag i
...
SELECT HEADERID,LINEID, LMSBOOKINGID INTO #TEMP2
FROM 
...
JOIN #TEMP2 TMP ON (Cast(LSQ_PBASE.ProspectID+','+PAE.ProspectActivityExtensionId as varchar(8000))=TMP.LMSBOOKINGID)
...
EXEC USP_ASM_PB_T_ENQUIRY_REFRESH;
EXEC USP_ASM_PB_HKT_DIM_MASTER_REFRESH;

----------------------------------Audit Log Target

SELECT @TargetCount2 = COUNT(DISTINCT FK_ENQUIRYDOCID) FROM ASM_TRIUMPH_ENQUIRY_STG;
