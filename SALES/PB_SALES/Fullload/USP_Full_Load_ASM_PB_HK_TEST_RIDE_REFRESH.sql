/****** Object:  StoredProcedure [dbo].[USP_Full_Load_ASM_PB_TEST_RIDE_REFRESH]    Script Date: 6/13/2025 5:24:05 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROC [dbo].[USP_Full_Load_ASM_PB_HK_TEST_RIDE_REFRESH] AS
BEGIN
...
/*******************************************HISTORY**************************************************/
------------------------------------------------------------------------------------------------------------------------------------------
TRUNCATE TABLE ASM_PB_HK_TEST_RIDE_FACT;
INSERT INTO ASM_PB_HK_TEST_RIDE_FACT
SELECT 
DEALERCODE,
SKU,
FK_DEALERCODE,
...
SELECT
 ...
 ----------------------------------------------------------------------------------------------------------------------------------------------
    ...
    AND CAST( LSQ_TESTRIDE.CREATEDON AS DATE)  >='2025-06-09'
  ) TMP
	GROUP BY 
...
SELECT
 ...
 ----------------------------------------------------------------------------------------------------------------------------------------------
	...
	--and LSQ_PEXTBASE.mx_BU_sub_type='TRM'
    AND LSQ_PEXTBASE.mx_Dealer_Assignment_Date  IS NOT NULL
    AND CAST( LSQ_TESTRIDE.CREATEDON AS DATE) between '2020-04-01' and '2025-06-08'
  ) TMP
	GROUP BY 
DEALERCODE, SKU,FK_DEALERCODE, FK_SKU,  FK_TYPE_ID,DATE,ENQUIRYLINEID,
 FK_ENQUIRYDOCID,COMPANYTYPE,BRANCHID,LASTUPDATEDDATETIME,IMPORTEDDATE,CDMS_BATCHNO ,TARGETQUANTITY,PERIODNAME
...
Delete from ASM_PB_HK_TEST_RIDE_FACT Where DATE>Cast(Getdate()-1 as date)  ;
--Dedup Process:
WITH CTE AS                  
 (                  
  SELECT *,                  
   ROW_NUMBER()OVER(PARTITION BY FK_ENQUIRYDOCID,ENQUIRYLINEID ORDER BY IMPORTEDDATE DESC)RNK                   
  FROM ASM_PB_HK_TEST_RIDE_FACT                 
 )                  
DELETE FROM CTE                  
 WHERE RNK<>1;  
...
update B set B.FK_SKU=C.PK_SKU from ASM_PB_HK_TEST_RIDE_FACT B INNER JOIN ASM_PB_HKT_PRODUCT_SKU_DIM C on (B.SKU=C.SKU_CODE)
update B set B.FK_MODEL=C.PK_Model_Code from ASM_PB_HK_TEST_RIDE_FACT B INNER JOIN ASM_PB_HKT_PRODUCT_DIM C on (B.MODELCODE=C.MODELCODE)
update B set B.FK_DEALERCODE=C.PK_DEALERCODE from [dbo].[ASM_PB_HK_TEST_RIDE_FACT] B INNER JOIN ASM_PB_HKT_DEALER_MASTER_DIM C on (B.DEALERCODE=C.DEALERCODE)



--------------------------Follow up salesview  ----------------------------

TRUNCATE TABLE ASM_PB_HK_FOLLOWUP_FACT;
INSERT INTO ASM_PB_HK_FOLLOWUP_FACT
SELECT 
DEALERCODE,
SKU,
FK_DEALERCODE,
...
SELECT
   ...
   Cast(0 as int) as FK_SKU,
   10010 As FK_TYPE_ID,
   Cast(FIRST_FOLLOWUP.FirstfollowupDate As Date) As DATE,
   FIRST_FOLLOWUP.PROSPECTACTIVITYEXTENSIONID as ENQUIRYLINEID,
   LSQ_PBASE.ProspectID as FK_ENQUIRYDOCID, 
   2 AS COMPANYTYPE,
   BM.BRANCHID as BRANCHID,
   LSQ_PBASE.ProspectId,
   getdate() as LASTUPDATEDDATETIME,
   FIRST_FOLLOWUP.Modifiedon AS IMPORTEDDATE,
   null as CDMS_BATCHNO,
   Cast(0 as decimal(19,0)) As TARGETQUANTITY,
   (LEFT(DATENAME( MONTH,LSQ_PEXTBASE.mx_Dealer_Assignment_Date),3)+'-'+Cast(Year(LSQ_PEXTBASE.mx_Dealer_Assignment_Date) as varchar(4))) As PERIODNAME,
   IV.CODE As COLOUR_CODE,
   ...
   FROM
	...
	LEFT JOIN (SELECT RelatedProspectID,FirstfollowupDate, FirstIsCustomerContacted,FirstFollowupScheduleDate,mx_custom_14,mx_custom_15,PROSPECTACTIVITYEXTENSIONID, Modifiedon  FROM (
select RelatedProspectID,mx_Custom_13 FirstIsCustomerContacted,mx_custom_3 as FirstFollowupScheduleDate,
createdon FirstfollowupDate, mx_custom_14 , mx_custom_15,PROSPECTACTIVITYEXTENSIONID, Modifiedon,
...
WHERE LSQ_PACTEXTBASE.ActivityEvent=12002 
	--and LSQ_PEXTBASE.mx_BU_sub_type='TRM'
    AND LSQ_PEXTBASE.mx_Dealer_Assignment_Date  IS NOT NULL
    AND CAST( FIRST_FOLLOWUP.FirstfollowupDate AS DATE)  >='2025-06-09'
  ) TMP
	GROUP BY 
DEALERCODE, SKU,FK_DEALERCODE, FK_SKU,  FK_TYPE_ID,DATE,ENQUIRYLINEID,
 FK_ENQUIRYDOCID,COMPANYTYPE,BRANCHID,LASTUPDATEDDATETIME,IMPORTEDDATE,CDMS_BATCHNO ,TARGETQUANTITY,PERIODNAME
,COLOUR_CODE, MODELCODE, FK_MODEL,FLAG, BaseFlag,TEHSILID,SalesPerson,LeadType,First_Source_Lead_Type
...
WHERE LSQ_PACTEXTBASE.ActivityEvent=12002 
	--and LSQ_PEXTBASE.mx_BU_sub_type='TRM'
    AND LSQ_PEXTBASE.mx_Dealer_Assignment_Date  IS NOT NULL
    AND CAST( FIRST_FOLLOWUP.FirstfollowupDate AS DATE)  between '2020-04-01' and '2025-06-08'
  ) TMP
	GROUP BY 
DEALERCODE, SKU,FK_DEALERCODE, FK_SKU,  FK_TYPE_ID,DATE,ENQUIRYLINEID,
 FK_ENQUIRYDOCID,COMPANYTYPE,BRANCHID,LASTUPDATEDDATETIME,IMPORTEDDATE,CDMS_BATCHNO ,TARGETQUANTITY,PERIODNAME
...
Delete from ASM_PB_HK_FOLLOWUP_FACT Where DATE>Cast(Getdate()-1 as date)  ;
...
update B set B.FK_SKU=C.PK_SKU from ASM_PB_HK_FOLLOWUP_FACT B INNER JOIN ASM_PB_HKT_PRODUCT_SKU_DIM C on (B.SKU=C.SKU_CODE)
update B set B.FK_MODEL=C.PK_Model_Code from ASM_PB_HK_FOLLOWUP_FACT B INNER JOIN ASM_PB_HKT_PRODUCT_DIM C on (B.MODELCODE=C.MODELCODE)
update B set B.FK_DEALERCODE=C.PK_DEALERCODE from [dbo].[ASM_PB_HK_FOLLOWUP_FACT] B INNER JOIN ASM_PB_HKT_DEALER_MASTER_DIM C on (B.DEALERCODE=C.DEALERCODE)
...
Leadtype,
First_Source_Lead_Type,
First_Mode_Source,First_Mode_SubSource
 from (
SELECT
   DISTINCT
   CM.CODE AS DEALERCODE,
 ...
 ----------------------------------------------------------------------------------------------------------------------------------------------
	...
	--and LSQ_PEXTBASE.mx_BU_sub_type='TRM'
    AND LSQ_PEXTBASE.mx_Dealer_Assignment_Date  IS NOT NULL
    AND CAST( LSQ_PEXTBASE.mx_Enquiry_update_date_and_time AS DATE)  >='2025-06-09'
  ) TMP
	GROUP BY 
DEALERCODE, SKU,FK_DEALERCODE, FK_SKU,  FK_TYPE_ID,DATE,ENQUIRYLINEID,
 FK_ENQUIRYDOCID,COMPANYTYPE,BRANCHID,LASTUPDATEDDATETIME,IMPORTEDDATE,CDMS_BATCHNO ,TARGETQUANTITY,PERIODNAME
...
SELECT
 ...
 ----------------------------------------------------------------------------------------------------------------------------------------------
	...
	--and LSQ_PEXTBASE.mx_BU_sub_type='TRM'
    AND LSQ_PEXTBASE.mx_Dealer_Assignment_Date  IS NOT NULL
    AND CAST( LSQ_PEXTBASE.mx_Enquiry_update_date_and_time AS DATE)  between '2020-04-01' and '2025-06-08'
  ) TMP
	GROUP BY 
DEALERCODE, SKU,FK_DEALERCODE, FK_SKU,  FK_TYPE_ID,DATE,ENQUIRYLINEID,
 FK_ENQUIRYDOCID,COMPANYTYPE,BRANCHID,LASTUPDATEDDATETIME,IMPORTEDDATE,CDMS_BATCHNO ,TARGETQUANTITY,PERIODNAME
...
WITH CTE AS                  
 (                  
  SELECT *,                  
   ROW_NUMBER()OVER(PARTITION BY FK_ENQUIRYDOCID,ENQUIRYLINEID ORDER BY IMPORTEDDATE DESC)RNK                   
  FROM ASM_PB_HK_TEST_RIDE_FACT  where FK_TYPE_ID='10011'               
 )                  
DELETE FROM CTE                  
 WHERE RNK<>1;  
