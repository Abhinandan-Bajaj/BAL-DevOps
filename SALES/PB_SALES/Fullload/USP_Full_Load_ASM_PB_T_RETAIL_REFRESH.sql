...
GO
ALTER PROC [dbo].[USP_Full_Load_ASM_PB_T_RETAIL_REFRESH] AS
BEGIN
...
/*******************************************HISTORY**************************************************/
--1.Retail Dim:
---TRUNCATE TABLE ASM_PB_T_RETAIL_DIM
...
-- AND COMPANY_MASTER.COMPANYSUBTYPE='Triumph' )
...
WHERE
--CAST(RH.DOCDATE AS DATE) BETWEEN '2021-01-01' AND '2023-10-31' AND 
CAST(RH.DOCDATE AS DATE) BETWEEN '2025-06-09' AND Cast(Getdate()-1 as date) AND 
RH.DOCTYPE=1000079
--AND RH.IMPORTEDDATE>(SELECT MAX(IMPORTEDDATE) FROM ASM_PB_T_RETAIL_DIM)
...
(CASE
	...
	ELSE 'Command Area' 
	END) AS SALESCHANNEL
	-- INTO ASM_TRIUMPH_RETAIL_DIM
FROM 
RETAIL_HEADER RH INNER JOIN COMPANY_MASTER 
ON (RH.COMPANYID=COMPANY_MASTER.COMPANYID AND COMPANY_MASTER.COMPANYTYPE = 2  AND COMPANY_MASTER.COMPANYSUBTYPE='Triumph' )
INNER JOIN BRANCH_MASTER ON (RH.BRANCHID=BRANCH_MASTER.BRANCHID)
INNER JOIN RETAIL_LINE ON (RH.HEADERID=RETAIL_LINE.DOCID)
LEFT JOIN BOOKING_LINE ON (RETAIL_LINE.BOOKINGDATALINEID=BOOKING_LINE.LINEID)
LEFT JOIN ENQUIRY_LINE ON (BOOKING_LINE.ENQUIRYDATALINEID=ENQUIRY_LINE.LINEID)
...
WHERE
--CAST(RH.DOCDATE AS DATE) BETWEEN '2021-01-01' AND '2023-10-31' AND 
CAST(RH.DOCDATE AS DATE) BETWEEN '2021-01-01' AND '2025-06-08' AND 
RH.DOCTYPE=1000079
--AND RH.IMPORTEDDATE>(SELECT MAX(IMPORTEDDATE) FROM ASM_TRIUMPH_RETAIL_DIM)
...
WHERE 
   -- CAST(RH.DOCDATE AS DATE) BETWEEN '2021-01-01' AND '2023-10-31'  AND
	...
	--AND RH.IMPORTEDDATE>(SELECT MAX(IMPORTEDDATE) FROM ASM_PB_T_RETAIL_STG)
GROUP BY
   CM.CODE,
   IM.CODE+IV.CODE,
...
NULL AS BOOKINGDATE,
NULL AS RETAILDATE
--INTO ASM_TRIUMPH_RETAIL_STG
FROM
   RETAIL_HEADER RH JOIN COMPANY_MASTER CM ON (RH.COMPANYID=CM.COMPANYID AND CM.COMPANYTYPE = 2  AND CM.COMPANYSUBTYPE='Triumph' )
   LEFT JOIN CONTACT_MASTER CN ON (RH.SALESPERSONID = CN.CONTACTID AND CN.ID = (SELECT MAX(CN1.ID) FROM CONTACT_MASTER CN1 WHERE CN.CONTACTID = CN1.CONTACTID))
   JOIN RETAIL_LINE RL ON (RH.HEADERID=RL.DOCID)
   JOIN ITEM_MASTER IM ON (IM.ItemId=RL.ItemID)
   LEFT JOIN ITEMVARMATRIX_MASTER IV ON (RL.VARMATRIXID=IV.ITEMVARMATRIXID)
...
WHERE 
   -- CAST(RH.DOCDATE AS DATE) BETWEEN '2021-01-01' AND '2023-10-31'  AND
	CAST(RH.DOCDATE AS DATE) BETWEEN '2021-01-01' AND '2025-06-08'  AND
	RH.DOCTYPE=1000079 /* IN(141,1000079,1000317,1012739,441) */
	--AND RH.IMPORTEDDATE>(SELECT MAX(IMPORTEDDATE) FROM ASM_TRIUMPH_RETAIL_STG)
GROUP BY
   CM.CODE,
   IM.CODE+IV.CODE,
   CAST(RH.DOCDATE as Date),  
...
--UNION
--SELECT * from ASM_PB_T_RETAIL_TARGET
